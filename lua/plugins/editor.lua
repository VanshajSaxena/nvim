return {
...

	{
		...
		keys = {
			...
			'Switch Buffer' },
			{ '<leader>fg', "<cmd>lua require('telescope.builtin').live_grep()<cr>",                                   desc =
			'Grep cwd' },
			{ '<leader>/',  "<cmd>lua require('telescope.builtin').live_grep()<cr>",                                   desc =
			'Grep cwd' },
			...
			'Buffers' },
			{ '<leader>ff', "<cmd>lua require('telescope.builtin').find_files({hidded = true})<cr>",
				                                                                                                           desc =
				'Find Files (root dir -> cwd)' },
			{ '<leader>fc', "<cmd> lua require('telescope.builtin').find_files({cwd = vim.fn.stdpath('config')})<cr>",
				                                                                                                           desc =
				'Find Files (cwd to vim.fn.stdpath("config"))' },
			--{ "<leader>fF", Util.telescope("files", { cwd = false }), desc = "Find Files (cwd)" },
			{ '<leader>fr', '<cmd>Telescope oldfiles<cr>',                                                             desc =
			'Recent' },
			{ '<leader>fR', "<cmd>lua require('telescope.builtin').oldfiles({cwd = vim.loop.cwd()})<cr>",
				                                                                                                           desc =
				'Recent (cwd)' },
			-- git
			{ '<leader>gc', '<cmd>Telescope git_commits<CR>',                                                          desc =
			'commits' },
			...
			'Auto Commands' },
			{ '<leader>sb', '<cmd>Telescope current_buffer_fuzzy_find<cr>',                                            desc =
			'Buffer' },
			{ '<leader>sc', '<cmd>Telescope command_history<cr>',                                                      desc =
			'Command History' },
			{ '<leader>sC', '<cmd>Telescope commands<cr>',                                                             desc =
			'Commands' },
			{ '<leader>sd', '<cmd>Telescope diagnostics bufnr=0<cr>',                                                  desc =
			'Document diagnostics' },
			{ '<leader>sD', '<cmd>Telescope diagnostics<cr>',                                                          desc =
			'Workspace diagnostics' },
			{ '<leader>uC', '<cmd>Telescope colorscheme enable_preview=true<cr>',                                      desc =
			'Telescope builtin' },
			{ '<leader>sp', '<cmd>Telescope builtin<cr>',                                                              desc =
			'Telescope builtin' },
			--{ "<leader>sg", Util.telescope("live_grep"), desc = "Grep (root dir)" },
			--{ "<leader>sG", Util.telescope("live_grep", { cwd = false }), desc = "Grep (cwd)" },
			{ '<leader>sh', '<cmd>Telescope help_tags<cr>',                                                            desc =
			'Help Pages' },
			{ '<leader>sH', '<cmd>Telescope highlights<cr>',                                                           desc =
			'Search Highlight Groups' },
			{ '<leader>sk', '<cmd>Telescope keymaps<cr>',                                                              desc =
			'Key Maps' },
			{ '<leader>sm', '<cmd>Telescope marks<cr>',                                                                desc =
			'Jump to Mark' },
			{ '<leader>sM', '<cmd>Telescope man_pages<cr>',                                                            desc =
			'Man Pages' },
			{ '<leader>so', '<cmd>Telescope vim_options<cr>',                                                          desc =
			'Options' },
			{ '<leader>sR', '<cmd>Telescope resume<cr>',                                                               desc =
			'Resume' },
			{ '<leader>sw', "<cmd>lua require('telescope.builtin').grep_string()<cr>",                                 desc =
			'Word (cwd)' },
